import random
import ast #VM added code for 1.a and 1.b
...
with open(RELATIVE_CONFIG_PATH+USER_COLLECTION+'.csv', 'r') as user_fh:
    for user_row in user_fh:
        ...
        useraxes_row = dict()
        if user_row:
            (username, email, role, useraxes_row) = user_row.split(',',3) #VM changes added code for access column
        #New changes device_id starts
        tmp_useraxes_row = ast.literal_eval(useraxes_row)#VM changed added for access column
        useraxes_data = []
        for data in tmp_useraxes_row:
            if data:
                (device_id,atype) = data.split('@')
            axes_data={'device_id' : device_id,'atype': atype}
            useraxes_data.append(axes_data)
        # New changes device_id starts
        user_data = {'username': username, 'email': email, 'role': role,'user_device_access': useraxes_data}#VM changed added for access column
